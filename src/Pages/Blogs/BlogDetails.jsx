...
const BlogDetails = () => {
    ...
    const { data: blog, refetch } = useQuery(['blog'], () =>
        axios.get(`${import.meta.env.VITE_APP_URL}/blogs/${id}`).then((res) => res.data)
    );

    
    useEffect(() => {
        fetch(`${import.meta.env.VITE_APP_URL}/comments/${id}`)
            .then((res) => res.json())
            .then((data) => {
    ...
    //     axios.get(`${import.meta.env.VITE_APP_URL}/comments/${id}`).then((res) => res.data)
    // );

    useEffect(() => {
        fetch(`${import.meta.env.VITE_APP_URL}/like/${id}`)
            .then((res) => res.json())
            .then((data) => {
    ...
    //     axios.get(`${import.meta.env.VITE_APP_URL}/like/${id}`).then((res) => res.data)
    // );
    ...
    const handleClick = () => {
        fetch(`${import.meta.env.VITE_APP_URL}/like?id=${id}&email=${user?.email}`, {
            method: 'POST',
            headers: {
    ...
    // Posting form data
    const muteFunc = async (data) => axios.post(`${import.meta.env.VITE_APP_URL}/comment/${id}`, data);
    ...
    const onSubmit = (data) => {
        ...
        fetch(`${import.meta.env.VITE_APP_URL}/comment/${id}`, {
            method: 'POST',
            headers: {
                'content-type': 'application/json',
            },
