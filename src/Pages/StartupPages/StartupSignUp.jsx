...
const StartupSignUp = () => {
  ...
  const muteFunc = async (userinfo) =>
    axios.post(`${import.meta.env.VITE_APP_URL_STARTUP}/api/auth/register`, userinfo);
  ...
  const { mutate } = useMutation(muteFunc, {
    onSuccess: (res) => {
      ...
      if (res.data.success) {
        axios
          .post(`${import.meta.env.VITE_APP_URL_STARTUP}/api/auth/login`, data)
          .then((response) => {
  ...
  const onSubmit = (data) => {
    ...
    const userInfo = {
      ...
      officePhone,
      role: 'startup',
      talentRequestPaymentDetails: { tier: 'tierFREE', id:null, transactionId: null,searchLimit:2 },
    };
