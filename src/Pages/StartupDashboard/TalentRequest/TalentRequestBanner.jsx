...
import TalentRequestPlanModal from './TalentRequestPlan/TalentRequestPlanModal';
...
const TalentRequestBanner = ({ setIsOpen, setOpen, open }) => {
  const { startupRequests,  startupDetails } = useContext(AuthContext);
  const findTalentHandler = () => {
    if (
      ...
      <div className="lg:grid lg:grid-cols-4 flex flex-col-reverse  justify-between h-full">
        <div className="col-span-2  lg:mt-10">
          ...
          <div className="mt-5 max-md:flex max-md:justify-center space-x-2">
            <button
              type="button"
              onClick={findTalentHandler}
              className="text-[#61C1FF] outline-none shadow-inner  px-5 bg-[#f8f1ff] rounded-lg py-2 max-lg:text-base lg:text-xl font-semibold"
            >
              Find me Talents !
            </button>
            <button
          ...
          {open && <TalentRequestPlanModal open={open} setOpen={setOpen} />}
          {/* for mobile version */}
          ...
          <div className="lg:grid hidden  mt-10  grid-cols-6    gap-4 ">
            ...
            {skillTalents?.map((item) => (
              <div
                key={Math.random()}
                className={`col-span-2 shadow-inner  rounded-xl   ${item?.bgColor}`}
              >
