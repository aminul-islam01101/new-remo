...
const TalentsFounds = ({ myRequests }) => {
  const navigate = useNavigate();
  const { startupDetails, results } = useContext(AuthContext);

  const handleMyRequest = () => {
    navigate('/dashboard/talent-request-history', { state: { data: myRequests?.myRequests } });
  };
// const results={
//   lastSearchResult:{}
// }
    ...
    <section className="mt-20 max-md:p-2 ">
          ...
          <div className="flex max-lg:bg-white w-full  py-5 2xl:gap-10 max-lg:rounded-2xl max-lg:shadow-xl max-lg:shadow-[#f5f8fd] justify-evenly lg:justify-center items-center">
            ...
            <div className="space-y-1.5">
              ...
              <h2 className="text-2xl 2xl:text-3xl font-bold text-[#333333]">
                {myRequests?.requestsInCurrentTier?.length || 0} /
      ...
      {/* talents found table */}
         ...
         {results?.lastSearchResult?.searchQuery?.details?.description && (
          <div>
          <TalentsFoundTable results={results} />
        </div>
         )}
    </section>
  );
