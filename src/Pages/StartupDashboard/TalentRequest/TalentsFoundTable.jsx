...
const TalentsFoundTable = ({ results }) => {
  const [isOpen, setIsOpen] = useState(false);
  const getStoredItems = () => JSON.parse(localStorage.getItem('selected-talents')) || [];
  const lsSelectedTalents = getStoredItems();

  const { user } = useSelector((state) => state.auth);

  

  const [selectedTalent, setSelectedTalent] = useState(lsSelectedTalents || []);
  const [isAllSelected, setIsAllSelected] = useState(false);

  const handleSelectTalent = (data) => {
    if (data?.isSelected === true) {
      setSelectedTalent([...selectedTalent, data]);
      const selectedT = [...lsSelectedTalents, data];
      setJob('selected-talents', selectedT);
    }
    if (data?.isSelected === false) {
      const datas = selectedTalent.filter((sel) => sel.id !== data.id);
      const removeSelection = lsSelectedTalents.filter((s) => s.result.email !== data.result.email);
      setJob('selected-talents', removeSelection);
  ...
  const handleSelectAllTalent = (data) => {
    if (data === true) {
      ...
      results?.lastSearchResult?.requiredTalentsInHistory.forEach((talent) => {
        const talentData = { result: talent, isSelected: true, id: talent._id };

        talentDataArr.push(talentData);
      });
      // setSelectedTalent({
      //   result: results?.lastSearchResult?.requiredTalentsInHistory,
      //   isSelected: true,
      // });
      setSelectedTalent(talentDataArr);
      setJob('selected-talents', []);
      setJob('selected-talents', talentDataArr);
  ...
  return (
    <>
      {!results?.lastSearchResult?.events?.length && (
        <section className="mt-10">
                    ...
                    <tbody>
                      {results?.lastSearchResult?.requiredTalentsInHistory?.length &&
                        results?.lastSearchResult?.requiredTalentsInHistory?.map((result) => (
                          <tr key={result._id} className="hover:bg-[#e3d5ff]">
                            ...
                            <td className="px-2 py-5 border-b border-gray-200 text-sm">
                              {isAllSelected ? (
                                ...
                                <input
                                  ...
                                  type="checkbox"
                                  checked={
                                    lsSelectedTalents.length &&
                                    lsSelectedTalents.some(
                                      (talent) => talent.result.email === result?.email
                                    )
                                  }
                                  disabled={isAllSelected}
                                  className="checkbox  focus:ring-0"
          ...
          {!gotAccess && (
            <div className="mt-10 flex justify-center">
              ...
              >
                Access Calender To Request Talent
              </button>
            </div>
          )}
          {/* request talent button */}
