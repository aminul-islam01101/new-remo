...
const TalentRequestsHistories = () => {
  ...
  const { handleCollapse, handleExpand, expanded, } = useContext(AuthContext);
  ...
  const { data: myRequests } = useQuery(['myRequests'], () =>
    axios
      .get(
        `${import.meta.env.VITE_APP_URL_STARTUP}/api/talent/my-requests?email=${
          user?.user.email
        }`
      )
      .then((res) => res.data)
  );
  const data = myRequests.myRequests;
  const { searchHistory: rawSearchHistory } = data || {};

  const searchHistory = rawSearchHistory?.sort(
    ...
    <div className="bg-[#f0f9ff] mt-6 py-2 pb-4  px-3 2xl:px-8 rounded-xl">
      ...
      <div className="space-y-3">
        {searchHistory?.map((talentHistory) => (
          <div key={Math.random()} className="bg-white rounded-xl ">
                ...
                <div>
                  ...
                  <div className=" flex flex-wrap mt-2 lg:mt-5 gap-1 lg:gap-2">
                    {talentHistory?.searchQuery?.selectedSkills?.map((skill) => (
                      <div key={Math.random()} className="flex gap-1 lg:gap-2 items-center rounded-md py-0.5 px-1.5  bg-[#b9f2ff]">
                ...
                <div className="space-y-2 col-span-2 flex  flex-col justify-center lg:items-end">
                  ...
                  <div className="flex justify-end flex-wrap gap-2">
                    {talentHistory?.searchQuery?.softSkills?.map((softSkill) => (
                      <div
                        key={Math.random()}
                        className="bg-[#b1edbf] rounded-md  flex flex-row gap-1.5 px-1.5 ml-0.5 2xl:px-2 py-0.5 items-center"
                      >
                        <span>
            ...
            {expanded.includes(talentHistory?._id) && (
              <>
                ...
                <div key={Math.random()} className="relative pb-5">
