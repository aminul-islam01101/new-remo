...
const Profile = () => {
  ...
  const { data: startupData } = useQuery(['startupData'], () =>
    axios
      .get(
        `${import.meta.env.VITE_APP_URL_STARTUP}/api/startup/startup-preview/${
            ...
            <div className="flex max-md:mt-3  col-span-3 items-center gap-2 ">
              ...
              >
                <SocialIcon
                  ...
                  url={startupData?.linkedIn ? startupData?.linkedIn : 'https://www.linkedin.com'}
                />
              </button>
              <button
                type="button"
              ...
              >
                <SocialIcon
                  style={{ height: 40, width: 40 }}
                  url={
                    startupData?.instagram ? startupData?.instagram : 'https://www.instagram.com'
                  }
                />
              </button>
              <button
              ...
              >
                <SocialIcon
                  style={{ height: 40, width: 40 }}
                  url={startupData?.twitter ? startupData?.twitter : 'https://twitter.com/'}
                />
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 gap-y-3 mt-3 ">
            {startupData?.domains?.length ? (
              startupData?.domains?.map((domain) => (
                <p
                  key={Math.random()}
                  className="border-[#19a5ff]  duration-300 ease-in hover:shadow-xl hover:shadow-[#d3cef2] text-sm font-semibold bg-white  px-2 py-0.5  border rounded-full"
                >
                  {domain}
                </p>
              ))
            ) : (
              <div key={Math.random()} className="flex justify-center  w-full mt-10">
