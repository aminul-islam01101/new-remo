...
const UpdateBlog = () => {
  ...
  const { data: blogCategories } = useQuery(['blogCategories'], () =>
    axios.get(`${import.meta.env.VITE_APP_URL}/blog-category`).then((res) => res.data)
  );

  const imgApI = process.env.REACT_APP_IMGBB_API;

  // Posting form data
  const muteFunc = async (data) => axios.post(`${import.meta.env.VITE_APP_URL}/add-blog`, data);
  ...
  const onSubmit = (data) => {
    ...
    fetch(url, {
      ...
      .then((imgData) => {
        ...
        // mutate(blogsInfo);
        fetch(`${import.meta.env.VITE_APP_URL}/update/${blog?._id}`, {
          method: 'PUT',
          headers: {
